...
INSERT INTO `departamentos` VALUES (4500,'MECANICA','MADRID','17.759.060');
...
SELECT * FROM EMPLEADOS WHERE cargo_emp='VENDEDOR' ORDER BY nombre DESC;
...
SELECT e.sal_emp, e.comision_emp, d.id_depto 
FROM empleados e, departamentos d
WHERE d.id_depto = 2000 ORDER BY e.comision_emp ASC;
...
-- del empleado.
SELECT e.nombre, e.sal_emp, e.comision_emp, d.id_depto, (sal_emp+comision_emp) AS 'Sueldo Bruto'
FROM empleados e, departamentos d
WHERE e.comision_emp<=e.sal_emp*0.3;

SELECT max(COMISION_EMP) AS 'Mas Poronga' FROM EMPLEADOS;
...
-- Hallar los departamentos que tienen más de tres empleados. 
-- Mostrar el número de empleados de esos departamentos.

SELECT d.id_depto, d.nombre_depto AS 'Departamento', e.nombre, e.id_emp, count(*)
FROM empleados e 
inner join departamentos d
on e.id_depto = d.id_depto
group by e.id_emp;

SELECT id_depto AS 'ID DEPARTAMENTO', count(id_emp) AS 'CANTIDAD EMPLEADOS'
FROM empleados
